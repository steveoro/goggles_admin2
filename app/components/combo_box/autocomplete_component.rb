...
module ComboBox
  ...
  #
  # Based on goggles_main AutocompleteComponent with the following differences:
  # - Adds server-side JWT support (accepts jwt parameter instead of client-side fetching)
  # - Adds bound_query_param support for additional query parameters
  ...
  class AutocompleteComponent < ViewComponent::Base
    def initialize(options = {})
      ...
      @jwt = options[:jwt] # Admin2: server-side JWT support
    end
    ...
    def data_attributes
      {
        ...
        'autocomplete-lookup-jwt-value' => @jwt # Admin2: server-side JWT support
      }
    end
