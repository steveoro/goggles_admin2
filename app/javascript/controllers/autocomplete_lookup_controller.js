...
/**
 * = Autocomplete Lookup controller (Admin2 bespoke version) =
 *
 * --> Admin2 bespoke version <--
 *
 * Based on goggles_main autocomplete_lookup_controller.js with the following differences:
 * - Accepts server-side JWT via jwt value (doesn't fetch client-side)
 * - Supports bound_query_param for additional query parameters
 * - Preserves Admin2-specific parameter naming for backward compatibility
 *
 * Original docs:
 *
 ...
 *
 * == Targets ==
 * @param {String} 'data-autocomplete-lookup-target': 'field'
 ...
 * @param {String} 'data-autocomplete-lookup-api-url-value'
 *                 base API URL for data request (w/o params);
 *                 when not set, AJAX setup will be skipped and html options will be used
 *
 * @param {String} 'data-autocomplete-lookup-placeholder-value'
 *                 localized help text for "Choose an option"
 *
 * @param {String} 'data-autocomplete-lookup-query-column-value'
 *                 query field name used in the API lookup call; defaults to 'name'
 *
 * @param {String} 'data-autocomplete-lookup-bound-query-value'
 ...
 *
 * @param {String} 'data-autocomplete-lookup-api-url2-value'
 ...
 *
 * @param {Boolean} 'data-autocomplete-lookup-free-text-value'
 ...
 *
 * @param {String} 'data-autocomplete-lookup-field-base-name-value'
 ...
 *
 * @param {String} 'data-autocomplete-lookup-jwt-value'
 *                 server-side JWT for API authentication (Admin2: passed from server, not fetched client-side)
 *
...
export default class extends Controller {
  ...
  static values = {
    ...
    jwt: String // Admin2: server-side JWT support
  }
  ...
  get jwt() {
    // Admin2: Use server-side JWT if provided
    if (this.hasJwtValue && this.jwtValue) {
  ...
  async loadRemoteOptions(query, callback) {
    ...
    const jwt = this.jwt
    if (!jwt) {
      console.warn('No JWT available for API call')
      callback([])
      return
    }
  ...
  async enrichMapDataWithDetails(mapData) {
    ...
    const jwt = this.jwt
    if (!jwt) {
      console.warn('No JWT available for entity details fetch')
      this.setHiddenFieldsValue(this.baseName, mapData)
      return
    }
