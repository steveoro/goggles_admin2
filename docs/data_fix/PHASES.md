...
The toggle stores the candidate snapshot in Phase 5 metadata. Candidates are displayed in their individual program cards with their MIR ID, timing, rank, lap count, and deletion reason. Each row has an autosaved selection toggle; ignored rows remain visible and are not deleted. Select All, Deselect All, and Deselect Zero-Timing controls update the snapshot without rediscovering candidates. Rows with minutes, seconds, and hundredths all equal to zero start ignored because they may be DSQs omitted from the source, but can be re-enabled manually. Phase 6 requires the normal commit confirmation plus a deletion-specific confirmation when active deletions exist, revalidates the snapshot, and fails closed if the database no longer matches the preview. SQL deletes dependent laps before MIRs inside the existing transaction. Selection remains in Phase 5 metadata for now; a temporary database candidate table is a possible future optimization only if profiling shows metadata size, autosave latency, or concurrency problems. Such a redesign would start in goggles_db with a source-scoped candidate table keyed by MIR ID and meeting/swimmer/team/program scope, indexed for source and selection queries, and would require migration, model, factory/spec, test dump regeneration, cleanup rules, and consumer propagation.
