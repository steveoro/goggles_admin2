...
RSpec.describe Grid::CreateNewButtonComponent, type: :component do
  ...
  shared_examples_for('a create new button with a proper Stimulus JS controller setup') do |namespace_base|
  ...
  context 'with valid default parameters,' do
    ...
    it_behaves_like('a create new button with a proper Stimulus JS controller setup', 'grid-edit')
  end
  ...
  context 'when overriding the base modal ID,' do
    ...
    it_behaves_like('a create new button with a proper Stimulus JS controller setup', 'subdetail')
  end
end
