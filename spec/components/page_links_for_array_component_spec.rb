...
RSpec.describe PageLinksForArrayComponent, type: :component do
  context 'with valid parameters,' do
    ...
    let(:sample_page) { 1 + (rand * 10).to_i }

    before(:each) do
      allow(controller).to receive(:params).and_return(
        {
          controller: 'import_queues', # (full CRUD)
          action: 'index',
          page: sample_page,
          per_page: sample_per_page
        }
    ...
    it 'renders the first page link' do
      expect(rendered_content.css('span.first a.page-link')).to be_present
    end

    it 'renders the previous page link' do
      expect(rendered_content.css('span.prev a.page-link')).to be_present
    end
